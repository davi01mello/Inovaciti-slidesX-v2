...
/**
 * Constantes que também cruzam a fronteira.
 *
 * Os TETOS de card e tópico são a regra de forma mais importante do produto, e
 * eles são aplicados dos DOIS lados (o servidor corta em normalize.ts, a UI não
 * deixa passar do teto). Se um lado disser 3 e o outro 5, o servidor devolve um
 * slide que a UI não sabe desenhar — e é o tipo de divergência que só aparece na
 * frente do cliente. Então o build falha aqui.
 */
const CONTRACT_CONSTS = ['MAX_CARDS', 'MAX_TOPICS', 'MAX_STATS', 'MAX_STEPS', 'MAX_COMPARE_POINTS', 'SLIDE_ICONS'];
...
function extractDeclaration(source, name) {
  ...
  const constMatch = clean.match(new RegExp(`export const ${name}\\s*=\\s*[^;]+;`));
  if (constMatch) return normalize(constMatch[0]);
...
const CONTRACT = [...CONTRACT_TYPES, ...CONTRACT_CONSTS];
...
console.log(
  `check:contract ok (${CONTRACT_TYPES.length} tipos + ${CONTRACT_CONSTS.length} constantes idênticos nos dois lados)`,
);
