...
export function WorkspaceHeader({
  ...
  return (
    <header className="flex h-[64px] flex-none items-center gap-3 border-b border-white/[0.04] bg-app-bg/85 px-5 backdrop-blur-md">
      ...
      {/* O pacto do produto, sempre à vista: o usuário cuida do conteúdo, o sistema cuida do design. */}
      <div className="pointer-events-none hidden items-center gap-2 rounded-full border border-brand/15 bg-brand/[0.04] py-1.5 pl-3 pr-4 lg:inline-flex">
        <Icon name="sparkles" size={12} className="text-brand" />
        <span className="whitespace-nowrap text-[11.5px] font-medium tracking-[-0.005em] text-ink-secondary">
          Você cuida do conteúdo, <span className="text-brand">eu cuido do design</span>
        </span>
      </div>
      ...
      <div className="flex items-center gap-1.5">
        ...
        >
          {canvaExporting ? 'Abrindo no Canva…' : 'Abrir no Canva'}
        </HeaderButton>
        <HeaderButton
          onClick={onExport}
          icon="import"
...
interface HeaderButtonProps {
  icon: 'presentations' | 'import' | 'compass';
  onClick: () => void;
...
function HeaderButton({ icon, onClick, children, disabled = false, title }: HeaderButtonProps) {
  return (
    <button
      type="button"
      onClick={onClick}
      disabled={disabled}
      title={title}
      className="inline-flex h-9 items-center gap-1.5 rounded-lg px-3 text-[13px] font-medium text-ink-secondary transition-all duration-150 hover:bg-white/[0.04] hover:text-ink disabled:cursor-not-allowed disabled:opacity-40"
    >
