import { isEmpty, toPlain, type RichText } from '@/lib/richText';
...
export function layoutGeneratedBlocks(layout: SlideLayout, blocks: GeneratedBlock[]): BlockRect[] {
  ...
  // O AutoShrinkText do editor segura qualquer excesso restante dentro do bloco.
  if (total > available) {
    gap = 0.02;
...
export function slideIsEmpty(s: Slide): boolean {
  return s.blocks.every((b) => (b.kind === 'bullets' ? b.items.every(isEmpty) : isEmpty(b.content)));
}

export function slideMainTitle(s: Slide): string {
  const titleBlock = s.blocks.find((b) => b.kind === 'title-1' || b.kind === 'title-2') as
    | { kind: string; content: RichText }
    | undefined;
  if (!titleBlock) return '';
  return toPlain(titleBlock.content);
}
