...
export function FloatingTextLayer({ blocks, editable, onContentChange, onMove, onDelete }: FloatingTextLayerProps) {
  ...
  return (
    <div className="pointer-events-none absolute inset-0">
      {blocks.map((block) => {
        // Caixa flutuante SEMPRE nasce com rect (makeFloatingTextBlock). O rect virou
        // opcional no tipo porque o texto do FLUXO não tem nenhum: quem o posiciona é
        // o motor de zonas. Aqui, sem rect, não há o que desenhar.
        ...
        if (!editable) {
          ...
          return (
            <TransformBox
              ...
              selected={false}
              onSelect={() => {}}
              style={{ ...textStyle, height: 'auto', minHeight: `${rect.height * 100}%` }}
            >
        ...
        return (
          <TransformBox
            ...
            kind="text"
            rect={rect}
            onChange={(next) => onMove(block.id, next)}
            interactive
