...
/**
 * Item de lista com identidade: sabe de qual bloco veio e qual índice ocupa lá.
 * É o que permite editar um card/linha DIRETO na composição (WYSIWYG).
 */
...
export interface ComposedSlide {
  ...
  support: TextBlock[];
  /** Itens de lista que viram cards ou linhas do template, com identidade de origem. */
  items: ComposedItem[];
 ...
 * Caixas de texto flutuantes (block.floating) ficam FORA do fluxo — são camada
 * livre por cima do template (ver FloatingTextLayer). */
export function composeSlide(fullSlide: Slide): ComposedSlide {
  const slide: Slide = {
    ...fullSlide,
    blocks: fullSlide.blocks.filter((b) => b.kind === 'bullets' || !b.floating),
  };
  const bulletBlocks = slide.blocks.filter((b): b is BulletsBlock => b.kind === 'bullets');
  const items = bulletBlocks
    .flatMap((b) => b.items.map((content, index): ComposedItem => ({ blockId: b.id, index, content })))
    .filter((item) => toPlain(item.content).trim().length > 0);
